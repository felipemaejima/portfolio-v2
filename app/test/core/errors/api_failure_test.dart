...
void main() {
  ...
  test('decide pelo code do contrato, não pelo status', () {
    expect(
      ...
      isA<ApiUnauthenticated>(),
    );
    expect(
      ApiFailure.from(
        _http(404, {'statusCode': 404, 'code': 'NOT_FOUND', 'message': 'x'}),
      ),
      isA<ApiNotFound>(),
    );
    expect(
      ApiFailure.from(
        _http(429, {'statusCode': 429, 'code': 'RATE_LIMITED', 'message': 'x'}),
      ),
      isA<ApiRateLimited>(),
    );
    final rejected = ApiFailure.from(
      _http(415, {
        'statusCode': 415,
  ...
  test('422 carrega details por campo', () {
    final f = ApiFailure.from(
      _http(422, {
        'statusCode': 422,
        'code': 'VALIDATION_FAILED',
        'message': 'Dados inválidos.',
        'details': {
          'email': ['deve ser um e-mail válido', 'obrigatório'],
        },
      }),
    );
    expect(f, isA<ApiValidation>());
    expect(
      (f as ApiValidation).field('email'),
      'deve ser um e-mail válido · obrigatório',
    );
