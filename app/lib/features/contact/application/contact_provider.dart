...
final contactLinksProvider = FutureProvider<List<ContactLinkDto>>(
  (ref) => ref.watch(contactRepositoryProvider).listLinks(),
);

/// Inbox do Admin; autoDispose porque só vive no painel.
final contactMessagesProvider =
    FutureProvider.autoDispose<List<ContactMessageDto>>(
      (ref) => ref.watch(contactRepositoryProvider).listMessages(),
    );
...
class ContactEditor {
  ...
  Future<void> createLink(ContactLinkInputDto body) =>
      _links(() => _repo.createLink(body));
  Future<void> updateLink(String id, ContactLinkInputDto body) =>
      _links(() => _repo.updateLink(id, body));
  Future<void> deleteLink(String id) => _links(() => _repo.deleteLink(id));
  Future<void> reorderLinks(List<String> ids) =>
      _links(() => _repo.reorderLinks(ids));
