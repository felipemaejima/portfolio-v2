  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (id == null) {
      return AdminPage(
        ...
        child: const _ProjectForm(initial: null),
      );
    }
    return AdminPage(
      ...
      child: AsyncValueView(
        ...
        data: (_) {
          ...
          return _ProjectForm(
            key: ValueKey(project.updatedAt),
            initial: project,
          );
        },
      ),
...
class _ProjectFormState extends ConsumerState<_ProjectForm> {
  ...
  late final _short = TextEditingController(
    text: widget.initial?.shortDescription,
  );
  late final _full = TextEditingController(
    text: widget.initial?.fullDescription,
  );
  late final _tech = TextEditingController(
    text: widget.initial?.technologies.join(', '),
  );
  ...
  Future<void> _removeImage(ProjectImageDto img) => _run(() async {
    await ref
        .read(projectsEditorProvider)
        .deleteImage(widget.initial!.id, img.id);
  ...
  Future<void> _reorderImages(int oldIndex, int newIndex) async {
    ...
    try {
      await ref
          .read(projectsEditorProvider)
          .reorderImages(widget.initial!.id, _images.map((i) => i.id).toList());
    } on Object catch (e) {
      if (!mounted) return;
        ...
        if (editing) ...[
          ...
          Row(
            children: [
              Expanded(
                child: Text(
                  '${l10n.gallery} (${_images.length}/$maxImagesPerProject)',
                  style: Theme.of(context).textTheme.titleMedium,
                ),
              ),
              OutlinedButton.icon(
                onPressed: _busy || _images.length >= maxImagesPerProject
                    ? null
                    : _addImages,
          ...
          if (_errors['files'] case final err?)
            Text(
              err,
            ...
            itemBuilder: (context, i) {
              ...
              return ListTile(
                ...
                trailing: IconButton(
                  ...
                  onPressed: _busy ? null : () => _removeImage(img),
                ),
              );
            },
          ),
