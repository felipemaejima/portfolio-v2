...
List<AdminDestination> adminDestinations(AppLocalizations l10n) => [
  ...
  AdminDestination(
    ...
    label: l10n.adminMessages,
  ),
];

int selectedDestination(List<AdminDestination> items, String location) {
  final i = items.lastIndexWhere(
    (d) =>
        d.path == '/admin' ? location == '/admin' : location.startsWith(d.path),
  );
  return i < 0 ? 0 : i;
}
